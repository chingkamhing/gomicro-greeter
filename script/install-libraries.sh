...
go get -v github.com/micro/protobuf/protoc-gen-go
...
#         - invoke "git checkout v1.4.3" to switch to v1.4.3
#         - invoke "make install" to go install github.com/golang/protobuf with v1.4.3
...
# * don't know how to check the version of protoc-gen-go, use file "protoc-gen-go.orig" as checker to determine if protoc-gen-go v1.0.0 is installed
#
PROTOC_GEN_GO=$(which protoc-gen-go)
PROTOC_GEN_GO_ORIG="${PROTOC_GEN_GO}.orig"
PROTOC_GEN_GO_VERSION="v1.0.0"
if [ "$PROTOC_GEN_GO" != "" ]; then
	# github.com/golang/protobuf properly installed
	if [ ! -f "$PROTOC_GEN_GO_ORIG" ]; then
		# believe the original protoc-gen-go is not backup yet, save it now
		echo "No $PROTOC_GEN_GO_ORIG. Install github.com/golang/protobuf v1.0.0 to fix gateway compile error."
		mv $PROTOC_GEN_GO $PROTOC_GEN_GO_ORIG
		...
		git checkout master
		cd -
		echo "protoc-gen-go is changed to $PROTOC_GEN_GO_VERSION and the original one is saved to $PROTOC_GEN_GO_ORIG."
	else
